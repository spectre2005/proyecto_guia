...
const TallasPanel = () => {
    ...
    const obtenerTallas = async () => {
        const { data } = await clienteAxios.get("/tallas");
        setTallas(Array.isArray(data) ? data : data.data || []);
    };
    ...
    const guardar = async (e) => {
        ...
        try {
            if (editando) {
                await clienteAxios.put(`/tallas/${editando.id}`, { nombre });
            } else {
                await clienteAxios.post("/tallas", { nombre });
            }
    ...
    const eliminar = async (id) => {
        ...
        try {
            await clienteAxios.delete(`/tallas/${id}`);
            obtenerTallas();
        } catch (error) {
            console.log(error);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-4xl font-bold text-blue-950">Tallas</h2>
                    <p className="text-gray-600 mt-2">Gestiona las tallas de productos.</p>
                </div>
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
                    <form onSubmit={guardar} className="bg-white w-full max-w-md rounded-xl p-6 relative">
                        ...
                        <h3 className="text-2xl font-bold text-blue-950 mb-5">
                            {editando ? "Editar talla" : "Nueva talla"}
                        </h3>
